...
//реализация доски 8 на 8(шашки только на черных плитках)
public class Board {

	public static final int INVALID = -1;// id точки, находящейся не на доске

	public static final int EMPTY = 0;// id пустой плитки
	...
	public void reset() {
		...
		// Восстановления состояния доски
	...
	public List<Point> find(int id) {
		...
		// нахождение всех черных плиток с соответствующим id
	...
	public static Point middle(int x1, int y1, int x2, int y2) {
		...
		if (x1 < 0 || y1 < 0 || x2 < 0 || y2 < 0 || // За доской
				x1 > 7 || y1 > 7 || x2 > 7 || y2 > 7) {
			return new Point(-1, -1);
		} else if (x1 % 2 == y1 % 2 || x2 % 2 == y2 % 2) { // Белая плитка(нан ней не может быть шашки)
			return new Point(-1, -1);
	...
	public static boolean isValidPoint(Point testPoint) {
		...
		if (x % 2 == y % 2) {//точка на белой плитке
			return false;
		}
